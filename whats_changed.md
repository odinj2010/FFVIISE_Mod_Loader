...
- **Field Map Texture Sequencer**: Added tracking of active field map folder names loaded from `flevel.lgp` (e.g. `trackin`, `ancnt1`) and sequenced texture tracking for loose custom field background overrides. Supported resolving field map texture folders mapped to `mods/<ModFolder>/field/<mapName>/<assetName>.png` according to the active load order priority.
