...
- **Safe Field Background SRV Hooking**: Added an `ID3D11Device::CreateShaderResourceView` (index 7) VMT hook to inject high-resolution field backgrounds. Implemented strict descriptor filtering (only 256x256 dynamic textures) and active field constraints (excluding character/chibi `.tex` files read from `flevel.lgp`) to completely eliminate texture pollution of character models (like eye textures), UI text, and the hand selector. Passed the original view descriptor (`pDesc`) to avoid binding crashes.
...
- **Loose Background & Interface Texture Redirections**: Generalised HookedFopen/HookedWfopen file parsing. Instead of restricting loose file interception strictly to paths containing `\ff7\workingdir\data\`, the loader now parses all files relative to `\data\`. This allows custom field background packs (such as raw `layout_pc\flevel\` assets) to be successfully intercepted and redirected. Added detours for `GetFileAttributesW`, `GetFileAttributesA`, `GetFileAttributesExW`, `CreateFileW`, and `CreateFileA` to intercept and redirect file/directory existence checks and direct open operations. Since the game checks if high-resolution directories or assets exist (either through attribute checks or attempting a direct open via `CreateFile`) before falling back to low-resolution LGP assets, mocking success and pointing these queries to active mod directories forces the engine to load custom backgrounds. Added automatic fallback mapping so if the mod contains classic `field\` folders, requests or existence/open checks for `layout_pc\flevel\` assets are seamlessly redirected to `field\` directories without manually restructuring the mod folders. Added explicit `[Redirect] File redirected:` and `CreateFile` redirected logs showing exactly where game background/loose files are located and what mod folder the replacement was loaded from.
