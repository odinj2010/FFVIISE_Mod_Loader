...
- **Battle Stage Texture Sequencer**: Implemented alphabetical-to-index mapping for `battle.lgp` reads (e.g. `abab` -> `STAGE01`) and sequenced texture tracking to dynamically reconstruct embedded battle environment filenames (e.g. `STAGE01_T00_00`, `STAGE01_T01_00`). Restricted stage index lookup to 0-89 range and implemented automated stage tracker resetting when `battle.lgp` is loaded or when a new stage master file (ending in `aa`) is read to ensure perfect texture synchronization across consecutive battles. Map texture creation sequences using active index vectors to handle gaps in file lists.
- **Field Map Texture Sequencer**: Added tracking of active field map folder names loaded from `flevel.lgp` (e.g. `trackin`, `ancnt1`) and sequenced texture tracking for loose custom field background overrides. Supported resolving field map texture folders mapped to `mods/<ModFolder>/field/<mapName>/<assetName>.png` according to the active load order priority. Uses sorted index vectors mapped directly to static texture creations to account for reversed creation orders between battle stages and field maps, resolving shifted sequence indexing.

### Fixed
- **Direct3D 11 Copy/Update Hooking & Map Bypass**: Re-implemented VMT hooks on `ID3D11DeviceContext::CopySubresourceRegion` (index 46), `ID3D11DeviceContext::UpdateSubresource` (index 48), `ID3D11DeviceContext::Map` (index 14), and `ID3D11DeviceContext::Unmap` (index 15) using a fast `std::unordered_set` lookup. Swaps both static (`Usage == 0`) and dynamic (`Usage == 2`) background textures with independent sequence counters. Bypasses dynamic mapping writes to a static dummy buffer to preserve HD dynamic textures from being overwritten, ensuring the field module draws HD backgrounds correctly regardless of rendering page paths.
