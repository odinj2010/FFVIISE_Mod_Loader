...
### Fixed
- **Direct3D 11 Copy/Update Hooking (Bypassed for Swapped Textures)**: Re-implemented VMT hooks on `ID3D11DeviceContext::CopySubresourceRegion` (index 46) and `ID3D11DeviceContext::UpdateSubresource` (index 48) using a fast `std::unordered_set` lookup to bypass GPU context copies only for our swapped HD textures. This protects our high-definition textures from being overwritten by low-resolution dynamic updates, resolving field map backgrounds rendering low-res without any driver lag.
