...
- **Loose Background & Interface Texture Redirections & 64-Bit Injection**: Generalised HookedFopen/HookedWfopen file parsing. Instead of restricting loose file interception strictly to paths containing `\ff7\workingdir\data\`, the loader now parses all files relative to `\data\`. This allows custom field background packs (such as raw `layout_pc\flevel\` assets) to be successfully intercepted and redirected. Added detours for `GetFileAttributesW`, `GetFileAttributesA`, `GetFileAttributesExW`, `CreateFileW`, and `CreateFileA` to redirect file open/existence queries. Added native 64-bit Direct3D 11 field background injection by hooking `ID3D11Device::CreateShaderResourceView` (index 7). We automatically capture the active field name (e.g., `trackin`) when the engine reads field map `.mim` files from `flevel.lgp`, tag the engine's 256x256 dynamic background quadrant textures at creation using DirectX private data (`GUID_BackgroundAssetName`), and swap the resulting Shader Resource View (SRV) with one referencing our loaded high-resolution static override texture. This completely removes the previous context-level hooks (`Map`, `Unmap`, `UpdateSubresource`) from the immediate context vtable, eliminating all risk of VMT collision, rendering pipeline lags, and side-effects, fully restoring battle model and UI text rendering visibility. Restrained dynamic texture private data tagging strictly to resources whose assigned names match active field or stage prefixes. This prevents other dynamic character model textures or UI templates from being incorrectly intercepted, preserving their dynamic update pipelines and restoring character model visibility. Added strict mutual exclusion between active field maps and active battle stages (clearing field maps when a battle loads and vice-versa) along with a strict 4-quadrant limit safeguard for field texture creation. This prevents dynamic battle textures or battle effects from being incorrectly hijacked as background quadrants. Added automatic fallback mapping so if the mod contains classic `field\` folders, requests or existence/open checks for `layout_pc\flevel\` assets are seamlessly redirected to `field\` directories without manually restructuring the mod folders. Added explicit `[Redirect] File redirected:` and `CreateFile` redirected logs showing exactly where game background/loose files are located and what mod folder the replacement was loaded from.
